...
local function screenshot(sel)
   ek.screenshot(config.screenshotdir, sel)
end
...
config.globalkeys = awful.util.table.join(
    ...
    -- M-. to focus master (found somewhere via ddg)
    awful.key({ modkey,           }, ".",
       function ()
          if awful and awful.client then
    ...
    -- screenshot
    awful.key({}, "Print", function() screenshot(true) end),
    ...
    -- Prompt
    awful.key({ modkey },            "r",     function () config.widgets["promptbox"].widget[mouse.screen]:run() end),

    awful.key({ modkey, "Control" }, "x",
              function ()
                  awful.prompt.run({ prompt = "Run Lua code: " },
                  config.widgets["promptbox"].widget[mouse.screen].widget,
                  awful.util.eval, nil,
...
for i = 1, 9 do
    config.globalkeys = awful.util.table.join(config.globalkeys,
        -- View tag only.
...
config.globalkeys = awful.util.table.join(config.globalkeys, ror.genkeys(modkey))
